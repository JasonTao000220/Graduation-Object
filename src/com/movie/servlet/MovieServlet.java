...
public class MovieServlet extends HttpServlet{
		...
		if("findMovieById".equals(method)){
			...
			try {
		        ...
		        if("addMovie".equals(param.get("method").toString())){
		    		...
		    		if(rs > 0) {
		    			map.put("code", 0);
		    			map.put("msg", "Successfully modified~");
		    		}else {
		    			map.put("code", 200);
		    			map.put("msg", "fail to edit~");
		    		}
		        }
	...
	private void findAllMovies(HttpServletRequest req, HttpServletResponse resp) throws IOException{
		...
		String type[] = {"Comedy", "Action", "Love", "Animation", "Science Fiction", "Thriller", "Adventure", "Crime", "Suspense"};
	...
	public void findMoviesByType(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		...
		if("0".equals(area) || "All".equals(area)){
			area = "";
		}
		if("0".equals(year) || "All".equals(year)){
			year = "";
		}
	...
	public void updateMovie(HttpServletRequest req, HttpServletResponse resp) throws Exception{
        ...
        //Parse the request and encapsulate each input item in the form into a FileItem object
        List<FileItem> fileItems = upload.parseRequest(req);
        // Iterating form data
        for (FileItem fileItem : fileItems) {
            //Determine whether the type of input is a normal input item or a file
            if (!fileItem.isFormField()) {
                //Uploading is a file, get the file name in the file upload field
                //Note that the file name obtained in IE or FireFox is not the same, in IE it is an absolute path, in FireFox it is just the file name
                String fileName = fileItem.getName();
                System.out.println(fileName);
                //Substring is a string interception, the return value is a intercepted string
                //lastIndexOf(".") is to check from right to left and get the string after.
                String ext = fileName.substring(fileName.lastIndexOf("."));
                //UUID.randomUUID().toString()It is a method of automatically generating primary keys provided by javaJDK. The only drawback of UUID is that the generated result string will be relatively long.
                String name = UUID.randomUUID()+ext;
                //Save the main content saved in the FileItem object to a specified file
    			String path = req.getRealPath("/file")+ "/upload/head/" + name;
                File file = new File(path);
